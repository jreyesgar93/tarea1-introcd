...
echo $"Primero analizamos el archivo ticdata2000.txt"
# Conteo de lineas y columnas del archivo de entrenamiento y de prueba
echo $"Este archivo contiene:" 

cat ticdata2000.txt|wc -l|  sed 's/$/ lineas/'

echo $"Las columnas estan separadas por \t (tabs)."
echo $"Para contar las columnas basta contar las palabras (no hay observaciones separadas por espacios)."
head -n 1 ticdata2000.txt | wc -w|  sed 's/$/ columnas/'

echo $"Después analizamos el archivo ticeval2000.txt"

echo $"Este archivo contiene:" 
cat ticeval2000.txt|wc -l|  sed 's/$/ lineas/'
head -n 1 ticeval2000.txt | wc -w|  sed 's/$/ columnas/'

echo $"Observamos que no trae la variable del resultado CARAVAN (No. 86)"
